...
bool MLTA::fuzzyTypeMatch(Type *Ty1, Type *Ty2, Module *M1, Module *M2) {
      ...
      if (FoundF && !FoundF->isIntrinsic()) {
        ...
        while (ContainersMap.find(CV) != ContainersMap.end()) {
          ...
          DBG << "[INSERT-INIT] Container type: " << *CTy
              ...
              << "\n\t --> FUNC: " << FoundF->getName()
              << "; Module: " << FoundF->getParent()->getName() << "\n";
...
void MLTA::confineTargetFunction(Value *V, Function *F) {
  ...
  getBaseTypeChain(TyChain, V, Complete);
  for (auto TI : TyChain) {
    DBG << "[INSERT-FUNC] Container type: " << *(TI.first)
        << "; Idex: " << TI.second << "\n\t --> FUNC:  " << F->getName()
        << "; Module: " << F->getParent()->getName() << "\n";
...
void MLTA::propagateType(Value *ToV, Type *FromTy, int Idx) {
  ...
  for (auto T : TyChain) {
    ...
    typeIdxPropMap[typeHash(T.first)][T.second].insert(
        hashidx_c(typeHash(FromTy), Idx));
    DBG << "[PROP] " << *(FromTy) << ": " << Idx << "\n\t===> " << *(T.first)
        << " " << T.second << "\n";
  }
}

void MLTA::intersectFuncSets(FuncSet &FS1, FuncSet &FS2, FuncSet &FS) {
  FS.clear();
  for (auto F : FS1) {
...
void MLTA::saveCalleesInfo(CallInst *CI, FuncSet &FS, bool mlta) {
...
void MLTA::printTypeChain(list<typeidx_t> &Chain) {
  ...
  for (list<typeidx_t>::iterator it = Chain.begin(); it != Chain.end(); ++it) {
    typeidx_t TI = *it;
...
// memory loads or GEP
bool MLTA::nextLayerBaseType(Value *V, list<typeidx_t> &TyList, Value *&NextV,
                             set<Value *> &Visited) {
...
bool MLTA::getTargetsWithLayerType(size_t TyHash, int Idx, FuncSet &FS) {
...
bool MLTA::findCalleesWithMLTA(CallInst *CI, FuncSet &FS) {
  ...
  while (ContinueNextLayer) {
    ...
    for (auto TyIdx : TyList) {
      ...
      DBG << "[CONTAINER] Type: " << *(TyIdx.first) << "; Idx: " << TyIdx.second
          << "\n";
